...
public class Customer implements Runnable {
    ...
    private void placeSaladOrder() {
        ...
        if (selectedSalad.getStock() > 0) {
            selectedSalad.decreaseStock();
            System.out.println("\n\033[32m[스레드 "+Thread.currentThread().getName().split("-")[3].replaceAll("\\D+", "") +"]\033[0m"+ 
            " - "+getName()+": 주문 완료! " + selectedSalad.getName() + "\033[31m (" + selectedSalad.getStock() + "개 남음)\033[0m");
        } else {
            System.out.println("\n\033[32m[스레드 "+Thread.currentThread().getName().split("-")[3].replaceAll("\\D+", "") +"]\033[0m"+
            "\033[31m - 재고 부족! " + selectedSalad.getName() + " 재고가 없습니다. 다시 주문해주세요.\033[0m");
        }
    }
    ...
    private void placeSaladSetOrder() {
        ...
        if (selectedSaladSet.getStock() > 0) {
            selectedSaladSet.decreaseStock();
            System.out.println("\n\033[32m[스레드 "+Thread.currentThread().getName().split("-")[3].replaceAll("\\D+", "")+"]\033[0m" + 
            " - " +getName()+": 주문 완료! "+ selectedSaladSet.getName() + "\033[31m (" + selectedSaladSet.getStock() + "개 남음)\033[0m");
        } else {
            System.out.println("\n\033[32m[스레드 "+Thread.currentThread().getName().split("-")[3].replaceAll("\\D+", "") +"]\033[0m"+
            "\033[31m - 재고 부족! " + selectedSaladSet.getName() + " 재고가 없습니다. 다시 주문해주세요.\033[0m");
        }
    }
}
